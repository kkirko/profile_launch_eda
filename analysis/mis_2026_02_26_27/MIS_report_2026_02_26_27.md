...
## Executive Summary
- В периоде зафиксировано **723** входов в чат-бот; до `cvEnhancedResult` дошли **521** кандидатов (**72.1%**).
...
- Users with non-empty `cvEnhancedResult`: **521** (72.1% от периода)
...
```
                                   stage  count  conversion_from_entry_%  conversion_from_prev_%
           1. Вход в чат-бот (createdAt)    723                    100.0                   100.0
                2. CV загружено (cvPath)    567                     78.4                    78.4
3. CV анализ завершён (cvAnalysisResult)    565                     78.1                    99.6
 4. CV enhanced готов (cvEnhancedResult)    521                     72.1                    92.2
                 5. Onboarding completed    521                     72.1                   100.0
```

### 2) Filtered cohort funnel (users included in this MIS slice)
```
                                   stage  count  conversion_from_entry_%  conversion_from_prev_%
           1. Вход в чат-бот (createdAt)    521                    100.0                   100.0
                2. CV загружено (cvPath)    521                    100.0                   100.0
3. CV анализ завершён (cvAnalysisResult)    521                    100.0                   100.0
 4. CV enhanced готов (cvEnhancedResult)    521                    100.0                   100.0
                 5. Onboarding completed    521                    100.0                   100.0
```
